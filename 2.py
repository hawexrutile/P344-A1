...
def DecaySimulator(N,a,T):                      #gives the no od decays per experiment
    N_0=N
    t=0
    Decays=0
    while t<T:                             #for each set of 10 sec
        u=0
        pop=[]
        while u<10:                        #This part of the code is the same as Q1  
...
surv1=lister(500,0.00004,300)
surv2=lister(500,0.00002,300)
#print(surv1)
'''
plt.plot(counter(surv1)[0],counter(surv1)[1])
plt.xlabel("Total decay -300s-->")
plt.ylabel("Repeats-->")
